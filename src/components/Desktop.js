...
import {POPUPS, VIDEOS, VIDEO_LINKS, VIDEO_LINKS_CH, UPDATE_VIDEOS, SOUNDS, POPUP_VIDEOS} from '../constants/constants'
...
class Desktop extends React.Component {
  constructor(props) {
    ...
    Object.keys(VIDEO_LINKS).forEach(key => { // Video refs
      this[`${key}en`] = React.createRef()
      this[key] = this[`${key}en`]
    })
    Object.keys(VIDEO_LINKS_CH).forEach(key => {
      this[`${key}ch`] = React.createRef()
  ...
  changeLanguage = language => {
    ...
    Object.keys(VIDEO_LINKS).forEach(key => {
      this[key] = this[`${key}${language}`]
    })
  }
  ...
  addPopup = ev => {
    ...
    if (this.state.activeVideo === VIDEOS.updateNotification0 || this.state.activeVideo === VIDEOS.updateNotification0 + 'ch' || this.state.updateCount > 0) return
  ...
  closeZoom = ev => {
    if (this.state.activeVideo === VIDEOS.zoom || this.state.activeVideo === VIDEOS.zoom + 'ch') {
      this.setState({
        isZoom: false
      })
  ...
  hideVideo = (ev, isEv = true) => {
    ...
    if (video.dataset.ref === VIDEOS.openingVideo || video.dataset.ref === VIDEOS.openingVideo + 'ch') {
      this.setState({
        activeVideo: null,
        isPlayingOpening: false
      })
  ...
  renderChineseVideo = (key, idx) =>(
    <div key={`${`${key}ch`}-${idx}`} className="video-parent">
      <video className="hidden-video" 
        width="500px" height="500px" 
        type="video/webm"  
        ref={this[`${key}ch`]}
      ...
      />
        ...
        <div className={`top-bar-container ${this.state.isPlayingClosing || this.state.hasUpdated ? 'invisible' : ''}`}>
          ...
          <div className="right-bar-container">
            ...
            {this.state.language === 'en' ? <div onClick={this.turnOnCaptions} className="icon captions"></div> : null}
    ...
    if (!this.state.isChrome || this.state.isMobile) return this.renderFirstScreen()
      ...
      return (
        <GlitchClip disabled={!this.state.isGlitching}>
          ...
          {Object.keys(VIDEO_LINKS_CH).map(this.renderChineseVideo)}
